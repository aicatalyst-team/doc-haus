...
// unpdf's pdf.js build (merged into one string). Always normalized (see sanitize.ts)
// so every consumer — indexing, the document/template text routes, and through them
// the agents' read-document tool — sees the same instruction-stripped text that
// chunk offsets were computed over.
